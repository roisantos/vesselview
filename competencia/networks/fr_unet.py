...
class block(nn.Module):
    def __init__(self, in_c, out_c,  dp=0.0, is_up=False, is_down=False, fuse=False):
        ...
        self.conv = conv(out_c, out_c, dp=self.dp)
        if self.is_up == True:
...
class FR_UNet(nn.Module):
    def __init__(self,  num_classes=1, num_channels=3, feature_scale=2,  dropout=0.0, fuse=True, out_ave=True):
        ...
        self.block1_3 = block(
            num_channels, filters[0],  dp= self.dropout, is_up=False, is_down=True, fuse=fuse)
        self.block1_2 = block(
            filters[0], filters[0],  dp= self.dropout, is_up=False, is_down=True, fuse=fuse)
        self.block1_1 = block(
            filters[0]*2, filters[0],  dp=self.dropout, is_up=False, is_down=True, fuse=fuse)
        self.block10 = block(
        ...
        self.linear = nn.Linear(5, num_classes, bias=False)
...
if __name__ == '__main__':
    ...
    # model = FR_UNet(num_classes=1, num_channels=3, feature_scale=2,  dropout=0.2)
    # output = model(x)
    # print(output.size())
